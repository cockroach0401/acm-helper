...
- 题目关联图片：本题可能包含参考题解截图或辅助说明图片，请仔细查阅提供的图片内容，结合参考题解理解本题解题思路。
- 这是用户学习的心得体会和代码,在生成题解时可以参考:{{reflection}}{{my_ac_code}}
...
### 2. 思路分析
从最直观的暴力解法出发，逐步分析其时间复杂度瓶颈所在，然后阐述优化思路的推导过程，说明为何选择特定的算法或数据结构，以及关键性质或引理的证明（若有）。如果提供了图片（如几何图解、公式截图等），请在分析中结合图片内容进行推导。
...
以启发式思维路径呈现题解。从拿到题目的第一直觉出发，描述观察到了哪些特殊结构、对称性或反常规律，如何从样例中捕捉到模式，如何通过手玩小数据建立猜想。强调“为什么会想到这个方法”而非“这个方法为什么对”，侧重类比、联想与经验迁移，例如“这个结构让人想起某类经典问题”或“如果把约束放宽会发生什么”。证明可以非形式化，用图示、手势性描述或极端情况验证来建立信心。代码部分保持正常可读性即可。
""".strip()
